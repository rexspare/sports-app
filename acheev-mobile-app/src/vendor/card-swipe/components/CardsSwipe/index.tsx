    ...
    const onCardSwiped = useCallback(
      (right: boolean) => {
        ...
        const onEndCardAnimation = () => {
          ...
          if (right) {
            // Swiping right, move to the previous card
            const newIndex = (index - 1 + cards.length) % cards.length;
            setIndex(newIndex);
            resetPosition((newIndex - 1 + cards.length) % cards.length);
          } else {
            // Swiping left, move to the next card
            const newIndex = (index + 1) % cards.length;
            setIndex(newIndex);
            resetPosition((newIndex + 1) % cards.length);
          }
      ...
      [index, cards.length, onSwiped, onSwipedRight, onSwipedLeft]
    );
