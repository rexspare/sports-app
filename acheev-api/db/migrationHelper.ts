...
import { DatabaseTable, Models } from "../src/shared/models";
...
/* Do not ever change this, this file is append only */
export function AddIdReferenceV1(table: Knex.CreateTableBuilder, columnName: string, referenceTable: DatabaseTable, index?: boolean): Knex.ReferencingColumnBuilder {
  let builder = table.integer(columnName).unsigned().notNullable().references("id").inTable(referenceTable.table);
  if (index === undefined ? true : index) builder.index();
  return builder;
}

/* Do not ever change this, this file is append only */
export function AddIdReferenceStringV1(table: Knex.CreateTableBuilder, columnName: string, referenceTable: DatabaseTable, index?: boolean): Knex.ReferencingColumnBuilder {
...
/* Do not ever change this, this file is append only */
export function AddUserIdReferenceV1(table: Knex.CreateTableBuilder, columnName?: string): Knex.ReferencingColumnBuilder {
  return AddIdReferenceStringV1(table, columnName ?? 'user_id', Models.USER, true).onDelete("RESTRICT");
}
/* Do not ever change this, this file is append only */
